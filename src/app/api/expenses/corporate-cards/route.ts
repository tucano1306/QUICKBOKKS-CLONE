  ...
  try {
    ...
    const card = await prisma.corporateCard.create({
      data: {
        ...
        expiryMonth: Number.parseInt(expiryMonth) || 12,
        expiryYear: Number.parseInt(expiryYear) || new Date().getFullYear() + 3,
        creditLimit: Number.parseFloat(creditLimit) || 0,
        availableCredit: Number.parseFloat(creditLimit) || 0,
        currentBalance: 0,
        status: 'ACTIVE',
        department,
        notes
  ...
  try {
    ...
    const card = await prisma.corporateCard.update({
      ...
      data: {
        ...
        creditLimit: updateData.creditLimit ? Number.parseFloat(updateData.creditLimit) : undefined,
