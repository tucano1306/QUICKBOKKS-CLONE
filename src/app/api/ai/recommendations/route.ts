import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { getServerSession } from 'next-auth'
import { NextRequest, NextResponse } from 'next/server'

export const dynamic = 'force-dynamic'

// ============================================================
// HELPER FUNCTIONS (reduce cognitive complexity of GET handler)
// ============================================================

function getImpactLevel(amount: number, highThreshold: number, mediumThreshold: number): 'High' | 'Medium' | 'Low' {
  if (amount > highThreshold) return 'High';
  if (amount > mediumThreshold) return 'Medium';
  return 'Low';
}

function computeMonthlyExpenses(expenses: any[]): number {
  const now = new Date();
  return expenses.reduce((sum, exp) => {
    const d = new Date(exp.createdAt);
    if (d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()) {
      return sum + exp.amount;
    }
    return sum;
  }, 0);
}

function buildOverdueRecommendation(overdueInvoices: any[], overdueAmount: number, priority: number) {
  if (overdueInvoices.length === 0) return null;
  const avgDaysOverdue = Math.round(
    overdueInvoices.reduce((sum, inv) => {
      const ms = Date.now() - new Date(inv.dueDate).getTime();
      return sum + Math.max(0, ms / (1000 * 60 * 60 * 24));
    }, 0) / overdueInvoices.length
  );
  return {
    id: `rec-${priority}`,
    category: 'Cobranza',
    title: 'Recuperar Cuentas por Cobrar Vencidas',
    description: `Tienes ${overdueInvoices.length} facturas vencidas por un total de $${overdueAmount.toLocaleString('es-MX')}. Implementar seguimiento activo podría recuperar este flujo.`,
    impact: getImpactLevel(overdueAmount, 50000, 20000),
    effort: 'Easy',
    priority,
    potentialRevenue: overdueAmount,
    timeToImplement: '1-2 semanas',
    status: 'New',
    steps: [
      `Revisar las ${overdueInvoices.length} facturas vencidas`,
      'Enviar recordatorios de pago personalizados',
      'Ofrecer planes de pago si es necesario',
      'Implementar cobro automatizado para futuras facturas',
      'Considerar políticas de crédito más estrictas'
    ],
    metrics: [
      { label: 'Facturas Vencidas', value: overdueInvoices.length.toString() },
      { label: 'Monto Total', value: `$${overdueAmount.toLocaleString('es-MX')}` },
      { label: 'Días Promedio Vencido', value: `${avgDaysOverdue} días` }
    ],
    roi: Math.round((overdueAmount / 1000) * 100)
  };
}

function buildExpenseRecommendation(sortedCategories: [string, number][], expenses: any[], priority: number) {
  if (sortedCategories.length === 0) return null;
  const [topName, topAmount] = sortedCategories[0];
  const potentialSavings = Math.round(topAmount * 0.15);
  const totalExpenses = expenses.reduce((s, e) => s + e.amount, 0);
  return {
    id: `rec-${priority}`,
    category: 'Optimización de Costos',
    title: `Optimizar Gastos en ${topName}`,
    description: `${topName} representa tu mayor gasto con $${topAmount.toLocaleString('es-MX')}. Una reducción del 15% podría ahorrar $${potentialSavings.toLocaleString('es-MX')} anuales.`,
    impact: getImpactLevel(potentialSavings, 30000, 10000),
    effort: 'Moderate',
    priority,
    potentialSavings,
    timeToImplement: '2-4 semanas',
    status: 'New',
    steps: [
      `Auditar todos los gastos de ${topName}`,
      'Identificar proveedores alternativos',
      'Negociar mejores tarifas con proveedores actuales',
      'Eliminar gastos redundantes',
      'Implementar controles de aprobación'
    ],
    metrics: [
      { label: 'Gasto Actual', value: `$${topAmount.toLocaleString('es-MX')}` },
      { label: 'Ahorro Potencial', value: `$${potentialSavings.toLocaleString('es-MX')}` },
      { label: '% del Total', value: `${((topAmount / (totalExpenses || 1)) * 100).toFixed(1)}%` }
    ],
    roi: Math.round((potentialSavings / 5000) * 100)
  };
}

function buildConcentrationRecommendation(
  topCustomers: any[],
  totalCustomerRevenue: number,
  totalCustomers: number,
  priority: number
) {
  if (topCustomers.length === 0 || totalCustomerRevenue === 0) return null;
  const topRevenue = topCustomers.reduce((sum, c) => sum + c.totalRevenue, 0);
  const concentration = (topRevenue / totalCustomerRevenue) * 100;
  if (concentration <= 50) return null;
  return {
    id: `rec-${priority}`,
    category: 'Diversificación',
    title: 'Reducir Concentración de Clientes',
    description: `El ${concentration.toFixed(0)}% de tus ingresos proviene de solo ${topCustomers.length} clientes. Alta dependencia representa riesgo de negocio.`,
    impact: 'High' as const,
    effort: 'Complex',
    priority,
    potentialRevenue: Math.round(totalCustomerRevenue * 0.2),
    timeToImplement: '3-6 meses',
    status: 'New',
    steps: [
      'Identificar nuevos segmentos de mercado',
      'Desarrollar estrategia de adquisición de clientes',
      'Crear ofertas para nuevos mercados',
      'Fortalecer relaciones con clientes actuales',
      'Implementar programa de referidos'
    ],
    metrics: [
      { label: 'Concentración Top 5', value: `${concentration.toFixed(1)}%` },
      { label: 'Ingresos Top 5', value: `$${topRevenue.toLocaleString('es-MX')}` },
      { label: 'Clientes Totales', value: totalCustomers.toString() }
    ],
    roi: 150
  };
}

function buildInventoryRecommendation(lowStockProducts: any[], totalProducts: number, priority: number) {
  if (lowStockProducts.length === 0) return null;
  return {
    id: `rec-${priority}`,
    category: 'Inventario',
    title: 'Reabastecer Productos con Stock Bajo',
    description: `${lowStockProducts.length} productos tienen inventario por debajo del punto de reorden. Evitar pérdida de ventas por desabasto.`,
    impact: 'Medium' as const,
    effort: 'Easy',
    priority,
    timeToImplement: '1 semana',
    status: 'New',
    steps: [
      'Revisar lista de productos con stock bajo',
      'Generar órdenes de compra',
      'Contactar proveedores principales',
      'Configurar alertas automáticas de reorden',
      'Evaluar demanda histórica para ajustar niveles'
    ],
    metrics: [
      { label: 'Productos Afectados', value: lowStockProducts.length.toString() },
      { label: 'Productos Totales', value: totalProducts.toString() }
    ],
    roi: 80
  };
}

function buildCashFlowRecommendation(totalBalance: number, monthlyExpenses: number, priority: number) {
  const runwayMonths = monthlyExpenses > 0 ? totalBalance / monthlyExpenses : 12;
  if (runwayMonths >= 3) return null;
  return {
    id: `rec-${priority}`,
    category: 'Flujo de Caja',
    title: 'Mejorar Reserva de Efectivo',
    description: `Tu reserva actual cubre solo ${runwayMonths.toFixed(1)} meses de operación. Se recomienda mantener mínimo 3 meses.`,
    impact: 'High' as const,
    effort: 'Moderate',
    priority,
    timeToImplement: '1-3 meses',
    status: 'New',
    steps: [
      'Acelerar cobranza de facturas pendientes',
      'Revisar y reducir gastos no esenciales',
      'Negociar términos de pago con proveedores',
      'Considerar línea de crédito como respaldo',
      'Establecer meta de ahorro mensual'
    ],
    metrics: [
      { label: 'Saldo Actual', value: `$${totalBalance.toLocaleString('es-MX')}` },
      { label: 'Gastos Mensuales', value: `$${monthlyExpenses.toLocaleString('es-MX')}` },
      { label: 'Runway', value: `${runwayMonths.toFixed(1)} meses` }
    ],
    roi: 200
  };
}

function buildAutomationRecommendation(invoiceCount: number, priority: number) {
  if (invoiceCount <= 10) return null;
  return {
    id: `rec-${priority}`,
    category: 'Automatización',
    title: 'Implementar Facturación Recurrente',
    description: 'Identificados clientes con patrones de facturación repetitivos. Automatizar puede ahorrar tiempo y reducir errores.',
    impact: 'Medium' as const,
    effort: 'Easy',
    priority,
    potentialSavings: 5000,
    timeToImplement: '1-2 semanas',
    status: 'New',
    steps: [
      'Identificar clientes con servicios recurrentes',
      'Configurar plantillas de factura automática',
      'Establecer calendarios de facturación',
      'Activar cobros automáticos',
      'Monitorear y ajustar según necesidad'
    ],
    metrics: [
      { label: 'Facturas Totales', value: invoiceCount.toString() },
      { label: 'Tiempo Estimado Ahorrado', value: '10+ hrs/mes' },
      { label: 'Ahorro Anual', value: '$5,000+' }
    ],
    roi: 120
  };
}

// ============================================================

/**
 * GET /api/ai/recommendations
 * Genera recomendaciones inteligentes basadas en datos reales
 */
export async function GET(req: NextRequest) {
  try {
    const session = await getServerSession(authOptions)

    if (!session?.user?.id) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const { searchParams } = new URL(req.url)
    const companyId = searchParams.get('companyId')
    const companyFilter = companyId ? { companyId } : {}
    const userCompanyFilter = { userId: session.user.id, ...companyFilter }

    const [invoices, expenses, customers, products, bankAccounts] = await Promise.all([
      prisma.invoice.findMany({
        where: userCompanyFilter,
        include: { customer: true },
        orderBy: { createdAt: 'desc' }
      }),
      prisma.expense.findMany({
        where: userCompanyFilter,
        include: { category: true },
        orderBy: { createdAt: 'desc' }
      }),
      prisma.customer.findMany({
        where: companyFilter,
        include: { invoices: { select: { total: true, status: true, createdAt: true } } }
      }),
      prisma.product.findMany({ where: companyFilter }),
      prisma.bankAccount.findMany({ where: userCompanyFilter })
    ])

    const overdueInvoices = invoices.filter(inv =>
      (inv.status === 'SENT' || inv.status === 'OVERDUE') && new Date(inv.dueDate) < new Date()
    )
    const overdueAmount = overdueInvoices.reduce((sum, inv) => sum + inv.total, 0)

    const expensesByCategory: Record<string, number> = {}
    expenses.forEach(exp => {
      const cat = exp.category?.name || 'Sin Categoría'
      expensesByCategory[cat] = (expensesByCategory[cat] || 0) + exp.amount
    })
    const sortedCategories = Object.entries(expensesByCategory)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 5)

    const customerRevenue = customers
      .map(c => ({ ...c, totalRevenue: c.invoices.reduce((sum, inv) => sum + inv.total, 0) }))
      .sort((a, b) => b.totalRevenue - a.totalRevenue)
    const topCustomers = customerRevenue.slice(0, 5)
    const totalCustomerRevenue = customerRevenue.reduce((sum, c) => sum + c.totalRevenue, 0)

    const lowStockProducts = products.filter((p: any) =>
      p.trackInventory && p.quantity !== undefined && p.quantity < (p.reorderPoint || 10)
    )

    const totalBalance = bankAccounts.reduce((sum, acc) => sum + acc.balance, 0)
    const monthlyExpenses = computeMonthlyExpenses(expenses)

    let counter = 1
    const recommendations = [
      buildOverdueRecommendation(overdueInvoices, overdueAmount, counter++),
      buildExpenseRecommendation(sortedCategories, expenses, counter++),
      buildConcentrationRecommendation(topCustomers, totalCustomerRevenue, customers.length, counter++),
      buildInventoryRecommendation(lowStockProducts, products.length, counter++),
      buildCashFlowRecommendation(totalBalance, monthlyExpenses, counter++),
      buildAutomationRecommendation(invoices.length, counter++)
    ].filter(Boolean)

    const summary = {
      totalRecommendations: recommendations.length,
      highImpact: recommendations.filter(r => r?.impact === 'High').length,
      mediumImpact: recommendations.filter(r => r?.impact === 'Medium').length,
      lowImpact: recommendations.filter(r => r?.impact === 'Low').length,
      totalPotentialSavings: recommendations.reduce((sum, r) => sum + ((r as any).potentialSavings || 0), 0),
      totalPotentialRevenue: recommendations.reduce((sum, r) => sum + ((r as any).potentialRevenue || 0), 0),
      avgROI: recommendations.length > 0
        ? Math.round(recommendations.reduce((sum, r) => sum + ((r as any).roi || 0), 0) / recommendations.length)
        : 0
    }

    return NextResponse.json({
      success: true,
      recommendations,
      summary,
      analyzedData: {
        invoices: invoices.length,
        expenses: expenses.length,
        customers: customers.length,
        products: products.length,
        bankAccounts: bankAccounts.length
      },
      generatedAt: new Date().toISOString()
    })

  } catch (error: any) {
    console.error('Error generating recommendations:', error)
    return NextResponse.json(
      { error: 'Failed to generate recommendations' },
      { status: 500 }
    )
  }
}

/**
 * POST /api/ai/recommendations
 * Actualiza el estado de una recomendación
 */
export async function POST(req: NextRequest) {
  try {
    const session = await getServerSession(authOptions)

    if (!session?.user?.id) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const body = await req.json()
    const { recommendationId, status, feedback } = body

    // En una implementación completa, guardaríamos esto en la base de datos
    // Por ahora retornamos éxito
    return NextResponse.json({
      success: true,
      message: `Recommendation ${recommendationId} updated to ${status}`,
      feedback
    })

  } catch (error: any) {
    console.error('Error updating recommendation:', error)
    return NextResponse.json(
      { error: 'Failed to update recommendation' },
      { status: 500 }
    )
  }
}
