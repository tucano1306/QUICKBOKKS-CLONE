                        ...
                        <TableCell>
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {customer.email && !customer.portalActive && (
                              <Button variant="ghost" size="sm" onClick={() => handleInviteToPortal(customer)} title="Invitar al portal" className="hover:bg-blue-50">
                                <Send className="h-4 w-4 text-blue-600" />
                              </Button>
                            )}

                            <Button variant="ghost" size="sm" onClick={() => { setSelectedCustomer(customer); setShowPermissionsModal(true); }} title="Configuración portal" className="hover:bg-purple-50">
                              <Settings className="h-4 w-4 text-purple-600" />
                            </Button>

                            {customer.portalActive && (
                              <Link href={`/customers/${customer.id}/activity`}>
                                <Button variant="ghost" size="sm" title="Ver actividad">
                            ...
                            <Link href={`/company/documents/upload?customerId=${customer.id}`}>
                              <Button variant="ghost" size="sm" title="Subir documentos" className="hover:bg-orange-50">
                                <Upload className="h-4 w-4 text-orange-600" />
                              </Button>
                            </Link>

                            <Link href={`/company/customers/transactions?customerId=${customer.id}`}>
                              <Button variant="ghost" size="sm" title="Ver transacciones" className="hover:bg-indigo-50">
                                <DollarSign className="h-4 w-4 text-indigo-600" />
                              </Button>
                            </Link>

                            <Link href={`/invoices?customerId=${customer.id}`}>
                              <Button variant="ghost" size="sm" title="Ver facturas" className="hover:bg-teal-50">
                                <Receipt className="h-4 w-4 text-teal-600" />
                              </Button>
                            </Link>

                            <Link href={`/customers/${customer.id}/notes`}>
                              <Button variant="ghost" size="sm" title="Notas y seguimiento" className="hover:bg-yellow-50">
                                <StickyNote className="h-4 w-4 text-yellow-600" />
                              </Button>
                            </Link>

                            <Link href={`/customers/${customer.id}/crm`}>
                              <Button variant="ghost" size="sm" title="Perfil CRM 360°" className="hover:bg-indigo-50">
                                <UserCircle className="h-4 w-4 text-indigo-600" />
                              </Button>
                            </Link>

                            <Button variant="ghost" size="sm" onClick={() => handleDeleteCustomer(customer.id)} title="Eliminar cliente">
